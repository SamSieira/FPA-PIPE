...
# -------------------------
# HELPER FUNCTIONS FOR ORF PREDICTION
# -------------------------
...
def predict_orfs(all_reads=None, ncbi=None, igan=None, orfs=None, cfg=None, out_path=None):
    ...
    print(f"Loaded {len(ncbi)} NCBI genes, {len(igan)} intergenic regions, {len(orfs)} ORFs.")

    # -------------------------
    # CLIP REGIONS TO SAMPLE LENGTH
    # -------------------------
    ...
    # -------------------------
    # REGION P-VALUES AND OBSERVED COUNTS
    # -------------------------
    ...
    plt.scatter(fpr[np.argmax(tpr - fpr)], tpr[np.argmax(tpr - fpr)],
                color='red', s=50, label='Youden Best Threshold')
    plt.xlabel('False Positive Rate (FPR)')
    plt.ylabel('True Positive Rate (TPR)')
    plt.title('ROC Curve for ORF Detection')
    plt.legend(loc='lower right')
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_path, "roc_curve.png"), dpi=300)
    plt.close()
    ...
    # -------------------------
    # ORF-level predictions (original)
    # -------------------------
    orf_results = []
    ...
    # NCBI COMPARISON
    # -------------------------
    gold_excel = cfg['output']['anubis']['results']
    sheet_name = cfg['settings']['sheet_name']
    model_name = cfg['settings']['model']

    print(f"Loading gold set from {gold_excel} [sheet={sheet_name}, model={model_name}]")
    gold_df = pd.read_excel(gold_excel, sheet_name=sheet_name)
    gold_df = gold_df[gold_df['model'] == model_name]

    gold_classes = dict(zip(gold_df['identifier'], gold_df['class']))
    categories = []
    skipped_ids_NCBI = []
    skipped_ids_IGAN = []
    # Process NCBI genes
    for rid, coords in ncbi.items():
        region_key = f"NCBI::{rid}"
        if region_key not in region_pvals:
            skipped_ids_NCBI.append(gid)
            continue
        avg_score = -np.log10(safe_mean_p(list(region_pvals[region_key].values())))
        detected = avg_score >= best_threshold_score
        gclass = gold_classes.get(rid, "NA")
        if detected:
            categories.append({"region_id": rid, "prediction": avg_score,
                            "num_insertions": sum(region_obs_counts[region_key].values()),
                            "type":"TP", "class": gclass})
        else:
            categories.append({"region_id": rid, "prediction": avg_score,
                            "num_insertions": sum(region_obs_counts[region_key].values()),
                            "type":"FN", "class": gclass})

    # Process IGAN regions
    for rid, coords in igan.items():
        region_key = f"IGAN::{rid}"
        if region_key not in region_pvals:
            skipped_ids_IGAN.append(gid)
            continue
        avg_score = -np.log10(safe_mean_p(list(region_pvals[region_key].values())))
        detected = avg_score >= best_threshold_score
        # IGAN regions are negatives: detected → FP, not detected → TN
        if detected:
            categories.append({"region_id": rid, "prediction": avg_score,
                            "num_insertions": sum(region_obs_counts[region_key].values()),
                            "type":"FP", "class":"NA"})
    ...
    print(f"Skipped {len(skipped_ids_NCBI)} gold identifiers not found in NCBI regions: {skipped_ids_NCBI[:10]}...")
    print(f"Skipped {len(skipped_ids_IGAN)} IGAN regions not found: {skipped_ids_IGAN[:10]}...") 

    # Save TP/FP/TN/FN in separate sheets
    tp_df = pd.DataFrame([c for c in categories if c['type']=='TP'])
    fp_df = pd.DataFrame([c for c in categories if c['type']=='FP'])
    tn_df = pd.DataFrame([c for c in categories if c['type']=='TN'])
    fn_df = pd.DataFrame([c for c in categories if c['type']=='FN'])

    with pd.ExcelWriter(os.path.join(out_path, "TP_TN_FP_FN.xlsx")) as writer:
        tp_df.to_excel(writer, sheet_name="TP", index=False)
        fp_df.to_excel(writer, sheet_name="FP", index=False)
        tn_df.to_excel(writer, sheet_name="TN", index=False)
        fn_df.to_excel(writer, sheet_name="FN", index=False)

    # -------------------------
    # BAR CHART: TP / TN / FP / FN with percentages & numbers
    # -------------------------
    summary_df = pd.DataFrame([c for c in categories if c['region_id'] in ncbi])
    classes = summary_df['class'].unique()
    x = np.arange(len(classes))
    width = 0.35

    tp_counts = [len(summary_df[(summary_df['class']==cls) & (summary_df['type']=='TP')]) for cls in classes]
    fn_counts = [len(summary_df[(summary_df['class']==cls) & (summary_df['type']=='FN')]) for cls in classes]

    fig, ax = plt.subplots(figsize=(8,6))
    ax.bar(x - width/2, tp_counts, width, label='Detected (TP)', color='skyblue')
    ax.bar(x + width/2, fn_counts, width, label='Undetected (FN)', color='lightcoral')

    # Add counts & percentages
    for i, cls in enumerate(classes):
        total = tp_counts[i] + fn_counts[i]
        if total == 0: 
            continue
        ax.text(i - width/2, tp_counts[i]+0.5, f"{tp_counts[i]}\n{tp_counts[i]/total*100:.1f}%", ha='center', va='bottom', fontsize=9)
        ax.text(i + width/2, fn_counts[i]+0.5, f"{fn_counts[i]}\n{fn_counts[i]/total*100:.1f}%", ha='center', va='bottom', fontsize=9)

    ax.set_xticks(x)
    ax.set_xticklabels(classes)
    ax.set_ylabel("Gene Count")
    ax.set_title("Detection of NCBI Genes by Class")
    ax.legend(loc='upper right')
    plt.tight_layout()
    plt.savefig(os.path.join(out_path, "ncbi_detected_vs_undetected_barplot.png"), dpi=300)
    plt.close()
    ...
    log_lines = [
        ...
        f"TPR at best: {tpr[np.argmax(tpr-fpr)]:.6f}",
        f"FPR at best: {fpr[np.argmax(tpr-fpr)]:.6f}",
        f"Skipped {len(skipped_ids_NCBI)} gold identifiers not in NCBI regions",
        f"Skipped {len(skipped_ids_IGAN)} IGAN regions not found",
        "",
    ...
    return df_orfs, region_pvals, categories, fpr, tpr, best_threshold_score
